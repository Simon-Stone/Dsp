...
{
	std::ofstream outFile("dsp_benchmark.csv", std::ios::out);
	...
	for(int j = 0; j < 100; ++j)
	{
		...
		std::cout << "Naive implementation: " << "\t\t" << duration.count() << " �s" << std::endl;
		...
		std::cout << "Modern DSP implementation: " << "\t" << duration_dsp.count() << " �s" << std::endl;
